...
You can also install the addon without installing its repository. To do so, get the [latest release](https://github.com/Sam-Max/plugin.video.jacktook/releases/download/v0.1.4/plugin.video.jacktook-0.1.6.zip) from github. Please note that, if there are any additional dependencies, they won't be resolved unless the repository is installed.
...
3. When using Jackett or Prowlarr with Debrid services don't add many trackers and avoid trackers that gives you torrent files instead of magnet links, and/or select if available on trackers options to retrieve magnets as priority, to improve search speed and results.
