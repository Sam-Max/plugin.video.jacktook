...
class StartupPreloader:
    ...
    def _preload(self):
            ...
            try:
                ...
                for item in top_items:
                    try:
                        ...
                        num_seasons = (
                            getattr(details, "number_of_seasons", 0)
                            if hasattr(details, "number_of_seasons")
                            else details.get("number_of_seasons", 0)
                        )
                        if num_seasons > 1:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            if trending_tv:
                executor.submit(_deep_preload_tv, trending_tv)
            if popular_tv:
                executor.submit(_deep_preload_tv, popular_tv)
