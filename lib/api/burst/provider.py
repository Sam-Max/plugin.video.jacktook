...
import threading
import time
from lib.api.burst.utils import bytes_to_str, str_to_bytes
    ...
    def register(self):
        ...
        if method in self._methods:
            try:
                ...
                self.logger.error(
                    "Failed running method '%s' with data '%s': %s",
                    method,
                    data_b64,
                    e,
                    exc_info=True,
            ...
            self.logger.error(
                "Unknown method provided '%s'. Expecting one of %s",
                method,
                self._methods.keys(),
            )
...
class ProviderListener(xbmc.Monitor):
    ...
    def onNotification(self, sender, method, data):
        kodilog(
            "Received notification with sender=%s, method=%s, data=%s",
            sender,
            method,
            data,
        )
        with self._lock:
            if method == self._method and self._waiting.get(sender, False):
                try:
                    ...
                    kodilog("Unable to get data from sender '%s': %s", sender, e)
                else:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if missing:
            kodilog("Provider(s) timed out: %s", ", ".join(missing), xbmc.LOGWARNING)
        return False
