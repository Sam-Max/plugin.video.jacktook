...
def colorful(dark, light):
    def decorate(f):
        ...
        def wrapper(
            ...
            format_dark=False,
            format_light=False,
            alignment_dark=False,
            alignment_light=False,
            timing_dark=False,
            timing_light=False,
            separator=False,
            dark_module=False,
...
def write_svg(
    ...
    def svg_color(clr):
        return (
            _color_to_webcolor(clr, allow_css3_colors=allow_css3_colors)
            if clr is not None
            else None
        )

    def matrix_to_lines_verbose():
        j = -0.5  # stroke width / 2
        invalid_color = -1
    ...
    if unit and omitsize:
        raise ValueError(
            f'The unit "{unit}" has no effect if the size (width and height) is omitted.'
        )
    ...
    need_background = (
        not is_multicolor and colormap[consts.TYPE_QUIET_ZONE] is not None and not draw_transparent
    )
    need_svg_group = scale != 1 and (need_background or is_multicolor)
    if is_multicolor:
        miter = matrix_to_lines_verbose()
    else:
        x, y = border, border + 0.5
    ...
    p = "<path{}{}".format(
        scale_info if not need_svg_group else "",
    ...
    for color, coord in coordinates.items():
        ...
        path += ' d="'
        path += "".join(
            "{moveto}{x} {y}h{l}".format(
                moveto=("m" if i > 0 else "M"),
                x=x,
                l=length,
...
_replace_quotes = partial(re.compile(rb'(=)"([^"]+)"').sub, rb"\1'\2'")
...
def as_svg_data_uri(
    ...
    **kw,
    ...
    return f"data:image/svg+xml{(';charset=' + encoding if not omit_charset else '')}," + encode(
        _replace_quotes(buff.getvalue())
    )
...
def write_svg_debug(
    ...
    with writable(out, "wt", encoding="utf-8") as f:
        ...
        write(
            '<style type="text/css"><![CDATA[ text { font-size: 1px; '
        ...
        for x, y, val in legend:
            write(f'<text x="{x + 0.2}" y="{y + 0.9}">{val}</text>')
        write("</g></svg>\n")
...
def write_eps(matrix, matrix_size, out, scale=1, border=None, dark="#000", light=None):
    ...
    with writable(out, "wt") as f:
        ...
        writeline("newpath")
        y = get_symbol_size(matrix_size, scale=1, border=0)[1] + border - 0.5  # .5 = linewidth / 2
...
def write_png(matrix, matrix_size, out, colormap, scale=1, border=None, compresslevel=9, dpi=None):
    ...
    is_greyscale = number_of_colors == 2 and all(
        clr in (transparent, black, white) for clr in palette
    )
    ...
    if not is_greyscale:  # PLTE
        ...
        if is_transparent:
            ...
            rgb_values = (
                _NAME2RGB.values()
                if len(palette[1]) == 3
                else ((*clr, 0) for clr in _NAME2RGB.values())
            )
    ...
    if number_of_colors > 2:
        ...
        color_index.update({0: color_index[qz_idx], 1: palette.index(clr_map[dark_idx])})
    ...
    with writable(out, "wb") as f:
        ...
        write(
            chunk(
                b"IHDR",
                pack(b">2I5B", width, height, png_bit_depth, png_color_type, 0, 0, 0),
            )
        )
        if dpi:
        ...
        if not is_greyscale:
            ...
            if len(palette[0]) > 3:  # Color with alpha channel is the first entry in the palette
                write(
                    chunk(
                        b"tRNS",
                        b"".join(pack(b">B", clr[3]) for clr in palette if len(clr) > 3),
                    )
                )
            elif is_transparent:
...
def write_pdf(
    ...
    y = get_symbol_size(matrix_size, scale=1, border=0)[1] + border - 0.5
    ...
    with writable(out, "wb") as f:
        ...
        object_pos.append(f.tell())
        writestr(
            f"{len(object_pos)} 0 obj <</CreationDate(D:{creation_date})"
...
def write_pam(matrix, matrix_size, out, scale=1, border=None, dark="#000", light="#fff"):
    ...
    row_filter = (
        invert_row_bits if colours is None else partial(row_to_color_values, colours=colours)
    )
    with writable(out, "wb") as f:
        write = f.write
        write(
...
def _color_to_rgb(color):
    ...
    if len(rgb) != 3:
        raise ValueError(
            f'The alpha channel {rgb[3]} in color "{color}" cannot be converted to RGB'
        )
    return rgb
...
def _color_to_rgba(color, alpha_float=True):
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f'Unsupported color "{color}". Neither a known web '
...
_ALPHA_COMMONS = {255: 1.0, 128: 0.5, 64: 0.25, 32: 0.125, 16: 0.625, 0: 0.0}
...
def _make_colormap(
    ...
    mt2color = {
        ...
        if alignment_light is not False
        else light,
