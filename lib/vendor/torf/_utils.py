...
def real_size(path):
    ...
    if os.path.isdir(os.path.realpath(path)):
        ...
        def onerror(exc):
            raise error.ReadError(getattr(exc, "errno", None), getattr(exc, "filename", None))
...
def filter_files(items, getter=lambda f: f, hidden=True, empty=True, exclude=(), include=()):
    """
    ...
    def is_excluded(
        ...
        if any(r.search(str(path)) for r in in_regexs) or any(
            fnmatch.fnmatch(str(path).casefold(), g.casefold()) for g in in_globs
        ):
            return False
        elif any(r.search(str(path)) for r in ex_regexs) or any(
            fnmatch.fnmatch(str(path).casefold(), g.casefold()) for g in ex_globs
        ):
            return True
        return False
    ...
    for item in items:
        ...
        # `basepath`
        if (
            (not hidden and is_hidden(relpath_without_base))
            or (not empty and os.path.exists(filepath) and real_size(filepath) <= 0)
            or is_excluded(relpath_with_base)
        ):
            continue
        else:
...
class MonitoredList(collections.abc.MutableSequence):
    ...
    def __eq__(self, other):
        if isinstance(other, type(self)):
            ...
            return len(other) == len(self._items) and all(item in self._items for item in other)
        else:
            return False
...
class File(os.PathLike):
    ...
    def __init__(self, path, size):
        if isinstance(path, str):
            ...
            raise ValueError(
                f"Path must be str, PathLike or Iterable, not {type(path).__name__}: {path}"
            )
    ...
    def __repr__(self):
        return f"{type(self).__name__}({str(self._path)!r}, size={self._size})"
...
class Trackers(collections.abc.MutableSequence):
    ...
    def __init__(self, tiers, callback=None):
        ...
        if isinstance(tiers, str):
            ...
            raise ValueError(f"Must be str or Iterable, not {type(tiers).__name__}: {tiers!r}")
        self._callback = callback
...
def download_http(url, timeout=60):
    try:
        ...
        try:
            ...
            msg = getattr(e, "msg", None) or getattr(e, "strerror", None) or "Failed"
...
def assert_type(obj, keys, exp_types, must_exist=True, check=None):
    ...
    if not key_exists_in_list_or_dict(key, obj):
        ...
        raise error.MetainfoError(
            f"{keychain_str}[{key!r}] must be {exp_types_str}, not {type_str}: {obj[key]!r}"
        )
